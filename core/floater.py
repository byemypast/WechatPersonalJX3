...
def APP_floater_main(player_id,state,msg,userdata,PlayerState):
	...
	if state==300:
		...
		core.game.sendlist(player_id,strcache)
		...
		if msg=="1":
			if floater_left<=0:
				core.game.sendstr(player_id,"您的捞瓶次数已用完，请投喂~")
			...
			core.game.response(1,player_id,"from state 301")
	elif state==302:
		...
		if (msg=="1")or(msg=='2'):
			...
			core.game.sendstr(player_id,"发送成功！【任意键】返回上一页")
			PlayerState[player_id] = 302
		elif msg=="3":
			needsending[player_id] = ""
